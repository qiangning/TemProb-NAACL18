...
public class TemporalInferenceWrapper {
    ...
    /*private class ArrayDimChecker<T>{
        public boolean check(T[][] objects, int dim1, int dim2){
            return objects!=null&&objects.length == dim1 && objects[0]!=null && objects[0].length == dim2;
        }
        public boolean check(T[][][] objects, int dim1, int dim2, int dim3){
            return objects!=null&&objects.length == dim1
                    &&objects[0]!=null&&objects[0].length==dim2
                    &&objects[0][0]!=null&&objects[0][0].length==dim3;
        }
    }*/

    public TemporalInferenceWrapper(int n_entitiy, int n_relation, double[][][] local_score, boolean[][] ignoreMap, int[][][] constraintMap, List<Triplet<Integer, Integer, List<Integer>>> transitivityMap) {
        ...
        this.constraintMap = new ArrayList<>();
        ...
        for(int i=0;i<n_entitiy;i++)
            Arrays.fill(result[i], TLINK.TlinkType.UNDEF.getTValueIdx());*/
    }
    public TemporalInferenceWrapper(int n_entitiy, int n_relation, double[][][] local_score, boolean[][] ignoreMap, List<int[][][]> constraintMap, List<Triplet<Integer, Integer, List<Integer>>> transitivityMap) {
        this.n_entitiy = n_entitiy;
        this.n_relation = n_relation;
        this.local_score = local_score;
        this.ignoreMap = ignoreMap;
    ...
    /*private boolean check(){
        ArrayDimChecker<Double> checker1 = new ArrayDimChecker<>();
         return checker1.check(local_score,n_entitiy,n_entitiy,n_relation);
    }*/
            ...
            for(int j=i+1;j<n_entitiy;j++){
                ...
                for(int[][][] constraintMap_single : constraintMap) {
                    ...
                    for (int k = 0; k < n_relation; k++) {
                        ...
                        //System.out.printf("%.1f*x_%d + ",coefs[k],vars[k]);
                    }
                    //System.out.printf(" = %d\n",constraintMap[i][j][n_relation]);
            ...
            for (int j = i + 1; j < n_entitiy; j++) {
                ...
                for(int k=i+1;k<n_entitiy;k++){
                    ...
                    for (Triplet<Integer,Integer,List<Integer>> triplet : transitivityMap) {
                        ...
                        //System.out.printf("%.1f*x_%d + %.1f*x_%d +",coefs[0],vars[0],coefs[1],vars[1]);
                        for(int p=0;p<m+2;p++){
                            if(vars[p]<0)
                                System.out.printf("Variable id less than 0: %d\n",vars[p]);
                            //System.out.printf("%.1f*x_%d + ",coefs[p],vars[p]);
                        }
